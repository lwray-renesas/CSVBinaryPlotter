...
The CSV Binary Plotter is a tool used to stream encoded binary data, CSV formatted, from a serial port or TCP port and plot it.
...
For another useful tool which allows viewing the generated CSV files from this tool, check out [CSVViewer](https://github.com/lwray-renesas/CSVViewer).
...
- [Using the Tool](#using-the-tool)
  - [File Save Behaviour](#file-save-behaviour)
- [Using the C Encoder Example](#using-the-c-encoder-example)
- [Protocol Breakdown](#protocol-breakdown)
  - [1. Stream Structure](#1-stream-structure)
  - [2. Metadata Format](#2-metadata-format)
    - [2.1 Metadata Types](#21-metadata-types)
    - [2.2 Metadata Definitions](#22-metadata-definitions)
      - [2.2.1 Column Names (#N)](#221-column-names-n)
      - [2.2.2 Column Types (#T)](#222-column-types-t)
      - [2.2.3 Endianness (#E)](#223-endianness-e)
    - [2.3 Metadata Requirements](#23-metadata-requirements)
    - [2.4 Metadata Request](#24-metadata-request)
  - [3. Data Row Format](#3-data-row-format)
  - [4. Binary Encoding and Escaping](#4-binary-encoding-and-escaping)
    - [4.1 Reserved Characters](#41-reserved-characters)
    - [4.2 Escape Mechanism](#42-escape-mechanism)
    - [4.3 Decoding Rule](#43-decoding-rule)
  - [5. Field Parsing](#5-field-parsing)
    - [5.1 Parsing Pipeline](#51-parsing-pipeline)
    - [5.2 Typed Conversion](#52-typed-conversion)
    - [5.3 Fixed-Point (Q Format) Support](#53-fixed-point-q-format-support)
- [License](#license)
...
```
├── C/src           # Example C encoder library for embedded systems
├── C/README.md     # Readme document for learning how to port the project
...
- **Electron App (`src/`)**
  - Handles serial or TCP communications
...
- A serial device sending properly encoded CSV binary data (see [C/README.MD](C/README.MD) for more information)
OR
- A TCP server transmitting CSV Binary Protocol data
...
1. DataConnect your embedded device to your machine via serial port (USB/UART).
...
Follow the instructions in [C/README.MD](C/README.MD).
...
| qx.y |              | Signed fixed point notation - see [5.3 Fixed-Point (Q Format) Support](#53-fixed-point-q-format-support)|
| uqx.y  |             | Unsigned fixed point notation - see [5.3 Fixed-Point (Q Format) Support](#53-fixed-point-q-format-support)|
...
#### 5.3 Fixed-Point (Q Format) Support

The protocol supports fixed-point numeric representation using Q format.

##### 5.3.1 Syntax
```
qX.Y    → signed fixed-point
uqX.Y   → unsigned fixed-point
```
...
uq1.15  → unsigned 16-bit fixed-point
```
---

##### 5.3.2 Storage Size Rule

Q format values are stored in the **smallest power-of-two integer size** that can contain all bits:

| Total Bits (X + Y) | Storage Size |
|------------------|-------------|
| ≤ 8              | 8-bit       |
| ≤ 16             | 16-bit      |
| ≤ 32             | 32-bit      |
...
Example:
```
q5.5   → 10 bits → stored as 16-bit
q12.4  → 16 bits → stored as 16-bit
q20.12 → 32 bits → stored as 32-bit
```
---

##### 5.3.3 Encoding Rule (Embedded Side)

For correct decoding, values **must be properly extended**:

**Signed Q (`qX.Y`):**
- Must be **sign-extended** to the full storage width

**Unsigned Q (`uqX.Y`):**
- Upper unused bits must be **zero**

Failure to follow these rules will result in incorrect decoded values.

---

##### 5.3.4 Decoding Rule (Host Side)

When decoding:
```
float_value = integer_value / (2^Y)
```
Example:
...
- `voltage` → signed Q8.8
- `current` → unsigned Q1.15
- `temperature` → signed Q16.16
